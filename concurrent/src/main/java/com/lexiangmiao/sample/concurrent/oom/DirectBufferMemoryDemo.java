...
/**
 * 配置参数：
 * -Xms10m -Xmx10m -XX:+PrintGCDetails -XX:+PrintGCDetails -XX:MaxDirectMemorySize=5m
 *
 * 故障现象：
 ...
 *
 *  ByteBuffer.allocate(capacity)         分配JVM堆内存，属于GC管辖范围，由于需要拷贝所以速度相对较慢
 *
 *  ByteBuffer.allocateDirect(capacity)   分配OS本地内存，不属于GC管辖范围，由于不需要内存拷贝所以速度相对较快。
 *
